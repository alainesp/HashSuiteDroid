...
PRIVATE int is_valid(char* user_name, char* ciphertext, char* unused, char* unused1)
{
	...
	if (ciphertext)
	{
		...
		pos++;
		if (!valid_base64_string(pos, 43))
			return FALSE;

		if (base64_to_num[pos[42]] & 0xF0)
			return FALSE;
...
PRIVATE sqlite3_int64 add_hash_from_line(ImportParam* param, char* user_name, char* ciphertext, char* unused, char* unused1)
{
	...
	if (ciphertext)
	{
		...
		return insert_hash_account1(param, user_name, all_ciphertext, SHA256CRYPT_INDEX);
	}
...
PRIVATE uint32_t get_binary(const unsigned char* ciphertext, void* binary, void* salt)
{
	...
	TO_BINARY(18, 28, 8);
	TO_BINARY(9, 19, 29);
	out[30] = ((uint32_t)base64_to_num[pos[0]]) | ((uint32_t)base64_to_num[pos[1]] << 6);
	out[31] = ((uint32_t)base64_to_num[pos[1]] >> 2) | ((uint32_t)base64_to_num[pos[2]] << 4);

	swap_endianness_array(binary, 8);
...
PRIVATE void binary2hex(const void* binary, const uint8_t* salt, unsigned char* ciphertext)
{
	...
	swap_endianness_array(tmp_state, 8);
	// Salt
...
void sha256_process_block(uint32_t* state, uint32_t* W);
...
PRIVATE void pre_cycle(const char* cleartext, uint32_t key_len, const crypt_sha256_salt* salt, char* buffer, uint32_t* out_state, char* p_bytes, char* s_bytes)
{
	...
	for (uint32_t j = key_len; j > 0; j >>= 1)
	{
		...
		{
			swap_endianness_array((uint32_t*)buffer, 16);
			sha256_process_block(out_state, (uint32_t*)buffer);
			buffer_len -= 64;
			num_blocks++;
			memcpy(buffer, buffer + 64, buffer_len);
		}
	}
	SHA256_END_CTX(out_state);
			...
			if (count_by_length[key_length] && (count_by_length[key_length] >= keys_in_parallel || last_iteration))
			{
				for (uint32_t current_salt_index = 0; current_salt_index < num_diff_salts; current_salt_index++)
				{
					...
					uint32_t need_2nd_block_by_g[4] = {
						(    key_length + 32                ) >= 56 ? 16 : 0,
						(2 * key_length + 32                ) >= 56 ? 16 : 0,
						(    key_length + 32 + salt->saltlen) >= 56 ? 16 : 0,
						(2 * key_length + 32 + salt->saltlen) >= 56 ? 16 : 0
					};

					memset(sha256_buffer, 0, 32 * 8 * sizeof(uint32_t) * keys_in_parallel);
					for (uint32_t i = 0; i < keys_in_parallel; i++)
					{
						...
						// Patterns---------------------------------------------------------------------------------------------
						//pattern[0]=alt pass
						memset(simple_buffer, 0, 128);
						memcpy(simple_buffer, p_bytes, key_length);
						simple_buffer[key_length] = 0x80;
						swap_endianness_array((uint32_t*)simple_buffer, 32);
						for (uint32_t j = 0; j < 32; j++)
							sha256_buffer[(32 * 0 + (j + 8)) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];
						...
						swap_endianness_array((uint32_t*)simple_buffer, 32);
						for (uint32_t j = 0; j < 32; j++)
							sha256_buffer[(32 * 1 + (j + 8)) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];
						...
						swap_endianness_array((uint32_t*)simple_buffer, 32);
						for (uint32_t j = 0; j < 32; j++)
							sha256_buffer[(32 * 2 + (j + 8)) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];
						...
						swap_endianness_array((uint32_t*)simple_buffer, 32);
						for (uint32_t j = 0; j < 32; j++)
							sha256_buffer[(32 * 3 + (j + 8)) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];
						//pattern[4]=pass alt
						memset(simple_buffer, 0, 128);
						memcpy(simple_buffer, p_bytes, key_length);
						swap_endianness_array((uint32_t*)simple_buffer, 32);
						for (uint32_t j = 0; j < 32; j++)
							sha256_buffer[(32 * 4 + j) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];
						...
						swap_endianness_array((uint32_t*)simple_buffer, 32);
						for (uint32_t j = 0; j < 32; j++)
							sha256_buffer[(32 * 5 + j) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];
						...
						for (uint32_t j = 0; j < (key_length + salt->saltlen + 3) / 4; j++)
							sha256_buffer[(32 * 6 + j) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];
						...
						for (uint32_t j = 0; j < (2 * key_length + salt->saltlen + 3) / 4; j++)
							sha256_buffer[(32 * 7 + j) * keys_in_parallel + i] = ((uint32_t*)simple_buffer)[j];

						// Size
						sha256_buffer[(32 * 0 + need_2nd_block_by_g[0] + 15) * keys_in_parallel + i] = (    key_length + 32                ) << 3;
						sha256_buffer[(32 * 1 + need_2nd_block_by_g[1] + 15) * keys_in_parallel + i] = (2 * key_length + 32                ) << 3;
						sha256_buffer[(32 * 2 + need_2nd_block_by_g[2] + 15) * keys_in_parallel + i] = (    key_length + 32 + salt->saltlen) << 3;
						sha256_buffer[(32 * 3 + need_2nd_block_by_g[3] + 15) * keys_in_parallel + i] = (2 * key_length + 32 + salt->saltlen) << 3;
						sha256_buffer[(32 * 4 + need_2nd_block_by_g[0] + 15) * keys_in_parallel + i] = (    key_length + 32                ) << 3;
						sha256_buffer[(32 * 5 + need_2nd_block_by_g[1] + 15) * keys_in_parallel + i] = (2 * key_length + 32                ) << 3;
						sha256_buffer[(32 * 6 + need_2nd_block_by_g[2] + 15) * keys_in_parallel + i] = (    key_length + 32 + salt->saltlen) << 3;
						sha256_buffer[(32 * 7 + need_2nd_block_by_g[3] + 15) * keys_in_parallel + i] = (2 * key_length + 32 + salt->saltlen) << 3;
						// end patterns------------------------------------------------------------------------------------------------------------
					}

					// Big cycle
					swap_endianness_array(state, 8 * keys_in_parallel);
					for (uint32_t k = 0, g_index = 0; k < salt->rounds; k++, g_index++)
					{
						uint32_t* pattern_buffer = sha256_buffer + 32 * keys_in_parallel * g[g_index];
						uint32_t need_2nd_block = need_2nd_block_by_g[g[g_index] & 3];
						if (k & 1)// Copy at end
						{
							uint32_t len = (pattern_buffer[(need_2nd_block + 15) * keys_in_parallel] >> 3) - 32;
							uint32_t len3 = len & 3;
							len /= 4;
							...
							{
								memcpy(pattern_buffer + len * keys_in_parallel, state, 32 * keys_in_parallel);
								for (uint32_t i = 0; i < keys_in_parallel; i++)
									pattern_buffer[(len + 8) * keys_in_parallel + i] = 0x80000000;
							}
						}
					...
					while (indx != NO_ELEM)
					{
						uint32_t* bin = ((uint32_t*)binary_values) + indx * 8;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#ifndef _M_X64
void sha256_process_block_c(void* state, void* tmp_block, const void* block)
{
	memcpy(tmp_block, block, 64);
	sha256_process_block(state, tmp_block);
}
void sha256_process_first_block_c(uint32_t* state, void* tmp_block, const void* block)
{
	state[0] = 0x6A09E667;
	state[1] = 0xBB67AE85;
	state[2] = 0x3C6EF372;
	...
	sha256_process_block_c(state, tmp_block, block);
}
PRIVATE void copy_pattern_c_code_1(uint32_t* pattern, const uint32_t* state)
{
	uint32_t buffer_value = pattern[0] & 0xff000000;
...
{
	uint32_t buffer_value = pattern[0] & 0xffff0000;
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		uint32_t state_value = state[j];
...
{
	uint32_t buffer_value = pattern[0] & 0xffffff00;
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		uint32_t state_value = state[j];
...
{
	V128_WORD buffer_value = V128_AND(V128_LOAD(pattern), V128_CONST(0xff000000));
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		V128_WORD state_value = V128_LOAD(state + j);
		V128_STORE(pattern, V128_OR(buffer_value, V128_SR(state_value, 8)));
		buffer_value = V128_SL(state_value, 24);
	}
...
{
	V128_WORD buffer_value = V128_AND(V128_LOAD(pattern), V128_CONST(0xffff0000));
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		V128_WORD state_value = V128_LOAD(state + j);
		V128_STORE(pattern, V128_OR(buffer_value, V128_SR(state_value, 16)));
		buffer_value = V128_SL(state_value, 16);
	}
...
{
	V128_WORD buffer_value = V128_AND(V128_LOAD(pattern), V128_CONST(0xffffff00));
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		V128_WORD state_value = V128_LOAD(state + j);
		V128_STORE(pattern, V128_OR(buffer_value, V128_SR(state_value, 24)));
		buffer_value = V128_SL(state_value, 8);
	}
	V128_STORE(pattern, V128_OR(buffer_value, V128_CONST(0x80)));
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// SSE2 code
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
PRIVATE void sha256_process_block_sse2(SSE2_WORD* state, SSE2_WORD* W, const SSE2_WORD* orig_W)
{
	SSE2_WORD A = state[0];
	SSE2_WORD B = state[1];
	SSE2_WORD C = state[2];
	...
	/* Rounds */
	W[ 0] = orig_W[ 0]; H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0x428A2F98), W[0 ]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 1] = orig_W[ 1]; G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0x71374491), W[1 ]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[ 2] = orig_W[ 2]; F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0xB5C0FBCF), W[2 ]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[ 3] = orig_W[ 3]; E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0xE9B5DBA5), W[3 ]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[ 4] = orig_W[ 4]; D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0x3956C25B), W[4 ]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[ 5] = orig_W[ 5]; C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0x59F111F1), W[5 ]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[ 6] = orig_W[ 6]; B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0x923F82A4), W[6 ]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[ 7] = orig_W[ 7]; A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0xAB1C5ED5), W[7 ]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));
	W[ 8] = orig_W[ 8]; H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0xD807AA98), W[8 ]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 9] = orig_W[ 9]; G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0x12835B01), W[9 ]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[10] = orig_W[10]; F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0x243185BE), W[10]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[11] = orig_W[11]; E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0x550C7DC3), W[11]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[12] = orig_W[12]; D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0x72BE5D74), W[12]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[13] = orig_W[13]; C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0x80DEB1FE), W[13]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[14] = orig_W[14]; B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0x9BDC06A7), W[14]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[15] = orig_W[15]; A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0xC19BF174), W[15]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));

	W[ 0] = SSE2_4ADD(W[ 0], R1(W[14]), W[9 ], R0(W[1 ])); H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0xE49B69C1), W[ 0]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 1] = SSE2_4ADD(W[ 1], R1(W[15]), W[10], R0(W[2 ])); G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0xEFBE4786), W[ 1]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[ 2] = SSE2_4ADD(W[ 2], R1(W[0 ]), W[11], R0(W[3 ])); F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0x0FC19DC6), W[ 2]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[ 3] = SSE2_4ADD(W[ 3], R1(W[1 ]), W[12], R0(W[4 ])); E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0x240CA1CC), W[ 3]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[ 4] = SSE2_4ADD(W[ 4], R1(W[2 ]), W[13], R0(W[5 ])); D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0x2DE92C6F), W[ 4]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[ 5] = SSE2_4ADD(W[ 5], R1(W[3 ]), W[14], R0(W[6 ])); C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0x4A7484AA), W[ 5]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[ 6] = SSE2_4ADD(W[ 6], R1(W[4 ]), W[15], R0(W[7 ])); B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0x5CB0A9DC), W[ 6]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[ 7] = SSE2_4ADD(W[ 7], R1(W[5 ]), W[0 ], R0(W[8 ])); A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0x76F988DA), W[ 7]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));
	W[ 8] = SSE2_4ADD(W[ 8], R1(W[6 ]), W[1 ], R0(W[9 ])); H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0x983E5152), W[ 8]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 9] = SSE2_4ADD(W[ 9], R1(W[7 ]), W[2 ], R0(W[10])); G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0xA831C66D), W[ 9]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[10] = SSE2_4ADD(W[10], R1(W[8 ]), W[3 ], R0(W[11])); F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0xB00327C8), W[10]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[11] = SSE2_4ADD(W[11], R1(W[9 ]), W[4 ], R0(W[12])); E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0xBF597FC7), W[11]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[12] = SSE2_4ADD(W[12], R1(W[10]), W[5 ], R0(W[13])); D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0xC6E00BF3), W[12]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[13] = SSE2_4ADD(W[13], R1(W[11]), W[6 ], R0(W[14])); C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0xD5A79147), W[13]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[14] = SSE2_4ADD(W[14], R1(W[12]), W[7 ], R0(W[15])); B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0x06CA6351), W[14]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[15] = SSE2_4ADD(W[15], R1(W[13]), W[8 ], R0(W[0 ])); A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0x14292967), W[15]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));
									 					  
	W[ 0] = SSE2_4ADD(W[ 0], R1(W[14]), W[9 ], R0(W[1 ])); H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0x27B70A85), W[ 0]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 1] = SSE2_4ADD(W[ 1], R1(W[15]), W[10], R0(W[2 ])); G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0x2E1B2138), W[ 1]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[ 2] = SSE2_4ADD(W[ 2], R1(W[0 ]), W[11], R0(W[3 ])); F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0x4D2C6DFC), W[ 2]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[ 3] = SSE2_4ADD(W[ 3], R1(W[1 ]), W[12], R0(W[4 ])); E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0x53380D13), W[ 3]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[ 4] = SSE2_4ADD(W[ 4], R1(W[2 ]), W[13], R0(W[5 ])); D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0x650A7354), W[ 4]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[ 5] = SSE2_4ADD(W[ 5], R1(W[3 ]), W[14], R0(W[6 ])); C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0x766A0ABB), W[ 5]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[ 6] = SSE2_4ADD(W[ 6], R1(W[4 ]), W[15], R0(W[7 ])); B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0x81C2C92E), W[ 6]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[ 7] = SSE2_4ADD(W[ 7], R1(W[5 ]), W[0 ], R0(W[8 ])); A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0x92722C85), W[ 7]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));
	W[ 8] = SSE2_4ADD(W[ 8], R1(W[6 ]), W[1 ], R0(W[9 ])); H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0xA2BFE8A1), W[ 8]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 9] = SSE2_4ADD(W[ 9], R1(W[7 ]), W[2 ], R0(W[10])); G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0xA81A664B), W[ 9]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[10] = SSE2_4ADD(W[10], R1(W[8 ]), W[3 ], R0(W[11])); F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0xC24B8B70), W[10]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[11] = SSE2_4ADD(W[11], R1(W[9 ]), W[4 ], R0(W[12])); E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0xC76C51A3), W[11]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[12] = SSE2_4ADD(W[12], R1(W[10]), W[5 ], R0(W[13])); D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0xD192E819), W[12]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[13] = SSE2_4ADD(W[13], R1(W[11]), W[6 ], R0(W[14])); C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0xD6990624), W[13]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[14] = SSE2_4ADD(W[14], R1(W[12]), W[7 ], R0(W[15])); B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0xF40E3585), W[14]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[15] = SSE2_4ADD(W[15], R1(W[13]), W[8 ], R0(W[0 ])); A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0x106AA070), W[15]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));
									 				 			
	W[ 0] = SSE2_4ADD(W[ 0], R1(W[14]), W[9 ], R0(W[1 ])); H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0x19A4C116), W[ 0]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 1] = SSE2_4ADD(W[ 1], R1(W[15]), W[10], R0(W[2 ])); G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0x1E376C08), W[ 1]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[ 2] = SSE2_4ADD(W[ 2], R1(W[0 ]), W[11], R0(W[3 ])); F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0x2748774C), W[ 2]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[ 3] = SSE2_4ADD(W[ 3], R1(W[1 ]), W[12], R0(W[4 ])); E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0x34B0BCB5), W[ 3]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[ 4] = SSE2_4ADD(W[ 4], R1(W[2 ]), W[13], R0(W[5 ])); D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0x391C0CB3), W[ 4]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[ 5] = SSE2_4ADD(W[ 5], R1(W[3 ]), W[14], R0(W[6 ])); C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0x4ED8AA4A), W[ 5]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[ 6] = SSE2_4ADD(W[ 6], R1(W[4 ]), W[15], R0(W[7 ])); B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0x5B9CCA4F), W[ 6]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[ 7] = SSE2_4ADD(W[ 7], R1(W[5 ]), W[0 ], R0(W[8 ])); A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0x682E6FF3), W[ 7]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));
	W[ 8] = SSE2_4ADD(W[ 8], R1(W[6 ]), W[1 ], R0(W[9 ])); H = SSE2_5ADD(H, R_E(E), SSE2_XOR(G, SSE2_AND(E, SSE2_XOR(F, G))), SSE2_CONST(0x748F82EE), W[ 8]); D = SSE2_ADD(D, H); H = SSE2_3ADD(H, R_A(A), SSE2_OR(SSE2_AND(A, B), SSE2_AND(C, SSE2_OR(A, B))));
	W[ 9] = SSE2_4ADD(W[ 9], R1(W[7 ]), W[2 ], R0(W[10])); G = SSE2_5ADD(G, R_E(D), SSE2_XOR(F, SSE2_AND(D, SSE2_XOR(E, F))), SSE2_CONST(0x78A5636F), W[ 9]); C = SSE2_ADD(C, G); G = SSE2_3ADD(G, R_A(H), SSE2_OR(SSE2_AND(H, A), SSE2_AND(B, SSE2_OR(H, A))));
	W[10] = SSE2_4ADD(W[10], R1(W[8 ]), W[3 ], R0(W[11])); F = SSE2_5ADD(F, R_E(C), SSE2_XOR(E, SSE2_AND(C, SSE2_XOR(D, E))), SSE2_CONST(0x84C87814), W[10]); B = SSE2_ADD(B, F); F = SSE2_3ADD(F, R_A(G), SSE2_OR(SSE2_AND(G, H), SSE2_AND(A, SSE2_OR(G, H))));
	W[11] = SSE2_4ADD(W[11], R1(W[9 ]), W[4 ], R0(W[12])); E = SSE2_5ADD(E, R_E(B), SSE2_XOR(D, SSE2_AND(B, SSE2_XOR(C, D))), SSE2_CONST(0x8CC70208), W[11]); A = SSE2_ADD(A, E); E = SSE2_3ADD(E, R_A(F), SSE2_OR(SSE2_AND(F, G), SSE2_AND(H, SSE2_OR(F, G))));
	W[12] = SSE2_4ADD(W[12], R1(W[10]), W[5 ], R0(W[13])); D = SSE2_5ADD(D, R_E(A), SSE2_XOR(C, SSE2_AND(A, SSE2_XOR(B, C))), SSE2_CONST(0x90BEFFFA), W[12]); H = SSE2_ADD(H, D); D = SSE2_3ADD(D, R_A(E), SSE2_OR(SSE2_AND(E, F), SSE2_AND(G, SSE2_OR(E, F))));
	W[13] = SSE2_4ADD(W[13], R1(W[11]), W[6 ], R0(W[14])); C = SSE2_5ADD(C, R_E(H), SSE2_XOR(B, SSE2_AND(H, SSE2_XOR(A, B))), SSE2_CONST(0xA4506CEB), W[13]); G = SSE2_ADD(G, C); C = SSE2_3ADD(C, R_A(D), SSE2_OR(SSE2_AND(D, E), SSE2_AND(F, SSE2_OR(D, E))));
	W[14] = SSE2_4ADD(W[14], R1(W[12]), W[7 ], R0(W[15])); B = SSE2_5ADD(B, R_E(G), SSE2_XOR(A, SSE2_AND(G, SSE2_XOR(H, A))), SSE2_CONST(0xBEF9A3F7), W[14]); F = SSE2_ADD(F, B); B = SSE2_3ADD(B, R_A(C), SSE2_OR(SSE2_AND(C, D), SSE2_AND(E, SSE2_OR(C, D))));
	W[15] = SSE2_4ADD(W[15], R1(W[13]), W[8 ], R0(W[0 ])); A = SSE2_5ADD(A, R_E(F), SSE2_XOR(H, SSE2_AND(F, SSE2_XOR(G, H))), SSE2_CONST(0xC67178F2), W[15]); E = SSE2_ADD(E, A); A = SSE2_3ADD(A, R_A(B), SSE2_OR(SSE2_AND(B, C), SSE2_AND(D, SSE2_OR(B, C))));

	state[0] = SSE2_ADD(state[0], A);
	state[1] = SSE2_ADD(state[1], B);
	state[2] = SSE2_ADD(state[2], C);
	state[3] = SSE2_ADD(state[3], D);
	state[4] = SSE2_ADD(state[4], E);
	state[5] = SSE2_ADD(state[5], F);
	state[6] = SSE2_ADD(state[6], G);
	state[7] = SSE2_ADD(state[7], H);
}
PRIVATE void sha256_process_first_block_sse2(SSE2_WORD* state, SSE2_WORD* W, const SSE2_WORD* orig_W)
{
	V128_STORE(state + 0, V128_CONST(0x6A09E667));
	V128_STORE(state + 1, V128_CONST(0xBB67AE85));
	V128_STORE(state + 2, V128_CONST(0x3C6EF372));
	V128_STORE(state + 3, V128_CONST(0xA54FF53A));
	V128_STORE(state + 4, V128_CONST(0x510E527F));
	V128_STORE(state + 5, V128_CONST(0x9B05688C));
	V128_STORE(state + 6, V128_CONST(0x1F83D9AB));
	V128_STORE(state + 7, V128_CONST(0x5BE0CD19));

	sha256_process_block_sse2(state, W, orig_W);
}
PRIVATE void crypt_utf8_coalesc_protocol_sse2(CryptParam* param)
{
	...
	sha256_process_block_func* kernels[] = { sha256_process_first_block_sse2, sha256_process_block_sse2 };
	crypt_utf8_coalesc_protocol_body(param, kernels, 4, copy_pattern_v128);
}
#endif
...
void sha256_process_first_block_avx(void* state, void* tmp_block, const void* block);
void sha256_process_block_avx(void* state, void* tmp_block, const void* block);
PRIVATE void crypt_utf8_coalesc_protocol_avx(CryptParam* param)
{
	copy_pattern_same_size_func* copy_pattern_v128[] = { copy_pattern_v128_1, copy_pattern_v128_2, copy_pattern_v128_3 };
	sha256_process_block_func* kernels[] = { sha256_process_first_block_avx, sha256_process_block_avx };
	crypt_utf8_coalesc_protocol_body(param, kernels, 4, copy_pattern_v128);
}
#endif
...
void sha256_process_first_block_avx2(void* state, void* tmp_block, const void* block);
void sha256_process_block_avx2(void* state, void* tmp_block, const void* block);
PRIVATE void copy_pattern_avx2_1(AVX2_WORD* pattern, const AVX2_WORD* state)
{
	AVX2_WORD buffer_value = AVX2_AND(AVX2_LOAD(pattern), AVX2_CONST(0xff000000));
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		AVX2_WORD state_value = AVX2_LOAD(state + j);
		AVX2_STORE(pattern + 0, AVX2_OR(buffer_value, AVX2_SR(state_value, 8)));
		buffer_value = AVX2_SL(state_value, 24);
	}
	AVX2_STORE(pattern, AVX2_OR(buffer_value, AVX2_CONST(0x800000)));
}
PRIVATE void copy_pattern_avx2_2(AVX2_WORD* pattern, const AVX2_WORD* state)
{
	AVX2_WORD buffer_value = AVX2_AND(AVX2_LOAD(pattern), AVX2_CONST(0xffff0000));
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		AVX2_WORD state_value = AVX2_LOAD(state + j);
		AVX2_STORE(pattern + 0, AVX2_OR(buffer_value, AVX2_SR(state_value, 16)));
		buffer_value = AVX2_SL(state_value, 16);
	}
	AVX2_STORE(pattern, AVX2_OR(buffer_value, AVX2_CONST(0x8000)));
}
PRIVATE void copy_pattern_avx2_3(AVX2_WORD* pattern, const AVX2_WORD* state)
{
	AVX2_WORD buffer_value = AVX2_AND(AVX2_LOAD(pattern), AVX2_CONST(0xffffff00));
	for (uint32_t j = 0; j < 8; j++, pattern++)
	{
		AVX2_WORD state_value = AVX2_LOAD(state + j);
		AVX2_STORE(pattern + 0, AVX2_OR(buffer_value, AVX2_SR(state_value, 24)));
		buffer_value = AVX2_SL(state_value, 8);
	}
	AVX2_STORE(pattern, AVX2_OR(buffer_value, AVX2_CONST(0x80)));
}
PRIVATE void crypt_utf8_coalesc_protocol_avx2(CryptParam* param)
{
	...
	sha256_process_block_func* kernels[] = { sha256_process_first_block_avx2, sha256_process_block_avx2 };
	crypt_utf8_coalesc_protocol_body(param, kernels, 8, copy_pattern_avx2);
}
#endif
...
Format sha256crypt_format = {
	"SHA256CRYPT",
	"SHA256-based crypt(3).",
	"$5$",
	MAX_KEY_SIZE,
	BINARY_SIZE,
	SALT_SIZE,
