...
// Copyright (c) 2017 by Alain Espinosa. See LICENSE.
...
#define SHA256_MAX_KEY_LENGHT	15
...
PRIVATE uint32_t get_binary(const unsigned char* ciphertext, void* binary, void* salt)
{
	...
	for (out_salt->saltlen = 0; *pos != '$' && out_salt->saltlen < SALT_SIZE; out_salt->saltlen++)
...
PRIVATE void crypt_utf8_coalesc_protocol_body(CryptParam* param, process_block_asm_func* kernel_asm, uint32_t keys_in_parallel, copy_pattern_same_size_func* copy_asm[])
{
	...
	while (continue_attack && param->gen(buffer, keys_in_parallel, param->thread_id))
	{
		...
		for (uint32_t i = 7 * keys_in_parallel; i < (8*keys_in_parallel); i++)
			if (buffer[i] > (SHA256_MAX_KEY_LENGHT << 3))
				buffer[i] = SHA256_MAX_KEY_LENGHT << 3;

		for (uint32_t current_salt_index = 0; current_salt_index < num_diff_salts; current_salt_index++)
		{
			crypt_sha256_salt salt = ((crypt_sha256_salt*)salts_values)[current_salt_index];
			...
			for (uint32_t k = 0, g_index = 0; k < 1000; k++, g_index++)
			{
				...
				if (k & 1)// Copy at end
				{
					...
					else// All keys had same lenght
					{
						uint32_t len = (pattern_buffer[14 * keys_in_parallel] >> 3) - 16;
						uint32_t len3 = len & 3;
						len /= 4;

						if (len3)// Note: Use 3 versions of functions because register shifts are incredible expensives
...
{
	/* Round 1 */
	SSE2_WORD a = SSE2_ADD(block[0], SSE2_CONST(0xd76aa477)); a = SSE2_ADD(SSE2_ROTATE(a, 7), SSE2_CONST(0));
	SSE2_WORD d = SSE2_3ADD(SSE2_XOR(SSE2_CONST(0), SSE2_AND(a, SSE2_CONST(0x77777777))), block[1 ], SSE2_CONST(0xf8fa0bcc)); d = SSE2_ADD(SSE2_ROTATE(d, 12), a);
	SSE2_WORD c = SSE2_3ADD(SSE2_XOR(SSE2_CONST(0), SSE2_AND(d, SSE2_XOR(a, SSE2_CONST(0)))), block[2 ], SSE2_CONST(0xbcdb4dd9)); c = SSE2_ADD(SSE2_ROTATE(c, 17), d);
	...
	state[0] = SSE2_ADD(a, SSE2_CONST(0));
	state[1] = SSE2_ADD(b, SSE2_CONST(0));
	state[2] = SSE2_ADD(c, SSE2_CONST(0));
	state[3] = SSE2_ADD(d, SSE2_CONST(0));
}
...
{
	// Generate code
	char* source = malloc(32 * 1024 * (1 + use_rules + SHA256_MAX_KEY_LENGHT + 1));
	source[0] = 0;
	// Header definitions
	...
	for (cl_uint i = 0; i <= SHA256_MAX_KEY_LENGHT; i++)
...
PRIVATE int ocl_protocol_common_init(OpenCL_Param* param, cl_uint gpu_index, generate_key_funtion* gen, gpu_crypt_funtion** gpu_crypt, oclKernel2Common* ocl_kernel_provider, int use_rules)
{
	...
	for (cl_uint i = 0; i < num_diff_salts; i++)
		if (((crypt_sha256_salt*)salts_values)[i].saltlen != 8)
		{
	...
	// Teoretical: 5.24M
	// HS by len : 4.50M
	if (!ocl_init_slow_hashes_ordered(param, gpu_index, gen, gpu_crypt, ocl_kernel_provider, use_rules, (use_rules ? 8 : 0) + 4, BINARY_SIZE, SALT_SIZE, ocl_gen_kernels, ocl_work_body, all_salt_lenght_8 ? 2 : 4, SHA256_MAX_KEY_LENGHT))
		return FALSE;
